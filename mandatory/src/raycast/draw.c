...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/23 18:01:03 by plinscho          #+#    #+#             */
/*   Updated: 2024/02/23 19:11:54 by plinscho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_line(t_game *game, t_line *line, t_img *img, t_img *s_img)
{
	...
	{
		line->text_y = (int)line->text_pos & (s_img->height - 1);
		line->text_pos += line->step;
		if (line->text_y <= 62)
			img_pix_put(img, line->x_start, i, \
				get_pixel_img(s_img, line->text_x, line->text_y));
		i++;
	}
	while (i < S_HEIGHT)
